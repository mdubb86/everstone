...
def svc(monkeypatch):
    ...
    monkeypatch.setattr("es.capabilities.cal.cal_support.calendar_policy",
                        lambda: ({"Partner's Calendar"}, ["Family", "Personal Calendar"]))
    monkeypatch.setattr("es.capabilities.cal.cal_support.resolve_calendar_id",
                        lambda s, name: {"Family": "fam@g", "Partner's Calendar": "al@g"}[name])
    return service


def test_add_refused_on_readonly_calendar(svc):
    res = runner.invoke(main.app, ["cal", "add", "X", "--calendar", "Partner's Calendar",
...
def test_delete_refused_on_readonly(svc):
    res = runner.invoke(main.app, ["cal", "delete", "eid", "--calendar", "Partner's Calendar"])
