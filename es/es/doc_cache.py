...
def page_image_path(adir: Path, page_no: int, image_no: Optional[int] = None) -> Path:
    ...
    `image_no` defaults to None for the WHOLE-PAGE raster es_doc_extract's
    image_pages parameter produces (`pNNN.png` — one file per requested page,
    unchanged from before this parameter existed). Passing `image_no` names
    one of POSSIBLY SEVERAL embedded images extracted from a single page
    ...
    by a SEPARATE call (doc_pdf.render(), invoked from docs.extract() when
    image_pages is given), never alongside convert()'s own per-image files
    for the same page.
    """
    if image_no is None:
